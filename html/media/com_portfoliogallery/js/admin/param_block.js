...
{     
    ...
    if (isNewProject) {
      window.location.href='index.php?option=com_portfoliogallery&view=portfoliogallery&task=portfoliogallery.addProject&sel='+sel+'&parentId='+parentId;
      return;        
    }
    ...
    if (par_images) {
        ...
        Joomla.submitbutton('portfoliogallery.save');
    }
...
function RemoveVideo(i, parentId) {
    var delEl = document.getElementById("editthisvideo_" + i + "_" + parentId);
...
function getImage(rootUrl, parentEl, updateId_, isNewProject,edit)
{
    if(isNewProject){
        ...
        var ImageSrc = location.protocol+'//'+rootUrl+'/'+document.getElementById('unique_name_button_'+updateId_).value;
    }
    else {
        var ImageSrc = location.protocol+'//'+rootUrl+'/'+document.getElementById('unique_name_button_'+parentEl).value;
    }
    updateId = updateId_;
